...
function check()
{
	if(checkUserName()&&checkPassword()&&confirmPwd()&&checkEmail()&&isPhone&&checkPhone()&&checkCodeNum()&&checkIdCard()&&validate())
		{
			return true;
		}
	else 
...
if (name==""){
	$("#nameDiv").next().css("display","block");
	$("#Usernamespan").html("* 用户名不能为空");
	$("#name").attr("class","input-text valid error");
	return false;
}
	...
	function show(boolean){
		if(boolean){
			alert("用户已经存在！");
			$("#nameDiv").next().css("display","none");
			$("#name").attr("class","input-text valid");
			return false;
		}
		else{
			$("#nameDiv").next().css("display","none");
...
{  
   ...
   /* else if(bValidate){
    	...
    	{
    	$("#PhoneNumDiv").next().css("display","none");
		isPhone=true;
    	}
