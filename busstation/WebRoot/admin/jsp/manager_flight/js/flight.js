...
function changeFlight(obj){
    ...
    var st = o.find("#St").text();
    o.find("#StInput").val(st);
    o.find("#StInput").show(); 
    
    var tr = o.find("#Remain").text();
    o.find("#RemainInput").val(tr);
    o.find("#RemainInput").show();
...
function submitFlight(obj,id){
	...
	var sr = o.find("#RemainInput").val();
	var st = o.find("#StInput").val();
	
	flight_update(fp,bp,tp,fr,br,tr,sr,st,id);   
...
/*
 * 异步执行数据库更新航班信息*/
function flight_update(fp,bp,tp,fr,br,tr,sr,st,id){
	layer.confirm('确认更改吗？', {
		btn: ['确认','取消'], 
		shade: false,
		closeBtn: 0
